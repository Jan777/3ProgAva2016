...
public class Mundo {
	...
	private void cargarMundo(String path) {
		String archivo = Utiles.archivoAString(path);
		String[] tokens = archivo.split("\\s+");
		ancho = Utiles.parseInt(tokens[0]);
		alto = Utiles.parseInt(tokens[1]);
		spawnX = Utiles.parseInt(tokens[2]);
		spawnY = Utiles.parseInt(tokens[3]);

		tiles = new int[ancho][alto];

		for (int y = 0; y < alto; y++) {
			for (int x = 0; x < ancho; x++) {
				tiles[x][y] = Utiles.parseInt(tokens[(x + y * ancho + 4)]);
			}
		}
	}
